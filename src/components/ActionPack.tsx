...
import { moves, staffCards, social, itemById, type Decision } from "@/lib/ryo";
import { Label, BigStat, gbp } from "./ui";

const orderMoves = moves.filter((m) => m.order && m.order.suggested > 0);

export function ActionPack({ decisions }: { decisions: Record<string, Decision> }) {
  const actioned = moves.filter((m) => decisions[m.id] === "actioned");
  const consider = actioned.length ? actioned : moves; // simulate all if nothing chosen yet
  ...
  return (
    <div>
      <header className="rise pb-6">
        <Label>Action pack · {social ? "week of 10 Aug" : ""}</Label>
      ...
      <div className="mb-5 rounded-xl border bg-[color:var(--color-surface)] rise rise-1">
        <div className="border-b px-6 py-4"><Label>Impact simulation · if you run {actioned.length ? "the actioned moves" : "all five"}</Label></div>
      ...
      <div className="grid grid-cols-1 gap-5 lg:grid-cols-2">
        <SupplierOrder total={orderTotal} />
...
function SupplierOrder({ total }: { total: number }) {
